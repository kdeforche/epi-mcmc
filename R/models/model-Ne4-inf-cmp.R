...
calclogl <- function(params) {
    ...
    logPriorP <- logPriorP + dbeta(k0, 1, 0.1, log=T)
...
fit.paramnames <- c("betaIn", "betaIs", "logHR", "HL", "DL",
                    "lockdownmort", "k0", "kt", "Tin0", "Tint")
keyparamnames <- c("betaIn", "betaIs", "R0", "Rt", "k0", "kt")
fitkeyparamnames <- c("betaIn", "betaIs", "k0", "kt", "Tin0", "Tint")

init <- c(1, 1, log(0.02), 10, 20, total_deaths_at_lockdown, 0.9, 0.5, 7, 2,
          rep(0.9, length(Es.time)))
scales <- c(0.3, 0.3, 0.05, 1, 1, total_deaths_at_lockdown / 20, 0.3, 0.3, 1, 1,
