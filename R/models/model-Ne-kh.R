...
simstep <- function(state, N, beta, a, gamma, h, k)
{
    i = state$i
    ...
    Ne = h * N
    if (state$I[i] > 0)
        Ne = Ne * (1 - k * (state$S[i - 6] - state$S[i]) / state$I[i])
...
calculateModel <- function(params, period)
{
    ...
    if (Tinc > 1) {
        for (i in (padding + 1):(padding + period)) {
            ...
            state <- simstep(state, N, beta, a, gamma, h, k)
...
calclogl <- function(params) {
    ...
    logPriorP <- logPriorP + dexp(k, 0.1, log=T)
...
fit.paramnames <- c("beta", "logHR", "logHRDR", "HL", "DL",
                    "Tinf", "ht", "Tinc", "lockdownmort", "k")
keyparamnames <- c("beta", "IFR", "Tinf", "h0", "ht", "Tinc", "R0", "Rt", "k")
fitkeyparamnames <- c("beta", "logHR", "logHRDR", "Tinf", "Tinc", "ht", "k")

init <- c(1, log(0.02), log(0.3), 10, 20, 5, 0.2, 8, total_deaths_at_lockdown, 1E-5,
          rep(0.9, length(Es.time)))
scales <- c(1, 0.05, 0.1, 1, 1, 0.3, 0.3, 0.3, total_deaths_at_lockdown / 20, 0.1,
