...
calclogp <- function(params) {
    ...
    logPriorP <- logPriorP + dnorm(fo8 * betao7, mean=0.5 * gamma, sd=0.2 * gamma, log=T)
    ...
    logPriorP <- logPriorP + dlnorm(fyifr, 0, log(1.1), log=T) # +/- 10%
    logPriorP <- logPriorP + dlnorm(fyhr, 0, log(1.1), log=T)
...
df_params <- data.frame(name = fit.paramnames,
                        min = c(2 * gamma, 2 * gamma, 2 * gamma,
                                ...
                                -15, 0.05 * gamma, 0.01 * gamma, 0.002 * gamma, 0.5, 0.5,
                                0.6, 0.6, 0.6),
                        max = c(8 * gamma, 8 * gamma, 8 * gamma,
                                ...
                                15, 3 * gamma, 3 * gamma, 0.5 * gamma, 2, 2,
                                1.1, 1.1, 1.1),
                        init = init)
