...
calcIb <- function(I, N, BS, h) {
    ...
    print(c("calcIb", I, N, BS, h, result))
    result
}

calch <- function(I, N, BS, Ib) {
    result = (1 - max(0, 1 - Ib * BS / N)^(1/BS)) * N / I
    print(c("calch", I, N, BS, Ib, result))
    result
}
...
calculateModel <- function(params, period)
{
    ...
    if (TRUE) {
        ...
        for (i in (padding + 1):(padding + period)) {
            ...
            Nb = exp(calcpar(i - data_offset, logNb0, logNbt, Es))
...
invTransformParams <- function(posterior)
{
    ...
    posterior$R0 = (posterior$betaIn0 + posterior$betaIs0) * posterior$Tinf
    posterior$Rt = (posterior$betaInt + posterior$betaIst) * posterior$Tinf
...
calclogl <- function(params) {
    ...
    if (betaIn0 < 1E-10 || betaInt < 1E-10 || betaIs0 < 1E-10 || betaIst < 1E-10) {
        return(-Inf)
    }
    ...
    logPriorP <- logPriorP + dnorm(betaIn0, mean=0, sd=1, log=T)
...
fit.paramnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "logHR", "HL", "DL",
                    "lockdownmort", "logNb0", "logNbt")
keyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "R0", "Rt", "Nb0", "Nbt")
fitkeyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "logNb0", "logNbt")

init <- c(1, 8, 3, 0.1, log(0.05), 15, 20, total_deaths_at_lockdown, log(N/10), log(N/10),
          rep(0.9, length(Es.time)))
scales <- c(0.3, 0.3, 0.3, 0.3, 0.05, 1, 1, total_deaths_at_lockdown / 20, 1, 1,
