...
calclogp <- function(params) {
    ...
    logPriorP <- logPriorP + dgamma(betay6/betay7, shape=GS, rate=GS, log=T)
    logPriorP <- logPriorP + dgamma(betao6/betao7, shape=GS, rate=GS, log=T)
    logPriorP <- logPriorP + dgamma(betayo6/betayo7, shape=GS, rate=GS, log=T)
    ...
    logPriorP <- logPriorP + dgamma(fyifr, shape=3, rate=3, log=T)
    logPriorP <- logPriorP + dgamma(fyhr, shape=3, rate=3, log=T)
...
init <- c(3.6 * gamma, 3.6 * gamma, 3.6 * gamma,
          ...
          G, 0.8 * gamma, 0.8 * gamma, 0.1 * gamma,
          0.8 * gamma, 0.8 * gamma, 0.1 * gamma, 1, 1)
...
df_params <- data.frame(name = fit.paramnames,
                        min = c(2 * gamma, 2 * gamma, 2 * gamma,
                                ...
                                3, 0.05 * gamma, 0.01 * gamma, 0.002 * gamma,
                                0.05 * gamma, 0.01 * gamma, 0.002 * gamma, 0.1, 0.1),
                        max = c(8 * gamma, 8 * gamma, 8 * gamma,
                                ...
                                12, 1.5 * gamma, 1.5 * gamma, 0.5 * gamma,
                                3 * gamma, 3 * gamma, 0.5 * gamma, 3, 3),
                        init = init)
