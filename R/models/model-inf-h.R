...
simstep <- function(state, N, beta, h, a, Tinf)
{
    i = state$i

    cr = max(0.01, 2 - exp(h * (N - state$S[i])/N))
    
    ## cr = max(0.01, 1 - h * ((N - state$S[i])/N)^2)
    Tinfr = Tinf * cr
    gamma = 1/Tinfr

    ## actually, should make an Ir : relevant infectious
    ## from E to I and Ir
    ## remove from Ir with gammar
    ## from I to R with gamma
    ##  will give the same results but we will have a more correct
    ##  visualization of I and R
    
    got_infected = beta * state$I[i] / N * state$S[i]
    got_infectious = a * state$E[i]
    got_removed = min(1, gamma) * state$I[i]
    ...
    ## if gamma > 1:
    ##   fraction of got_infectious skips I and goes directly to R
    if (gamma > 1) {
        ## tinfr = 0.3 -> gamma = 3
        ## tinfr = 0.25 -> gamma = 4
        ##  2/3 of got_infectious added to got_removed
        ##  3/4
        fr <- (gamma - 1) / gamma
        deltaI <- deltaI - fr * got_infectious
        deltaR <- deltaR + fr * got_infectious
    }
...
calculateModel <- function(params, period)
{
    ...
    if (Tinc > 1) {
        for (i in (padding + 1):(padding + period)) {
            h = calcpar(i - data_offset, h0, ht, Es)

            state <- simstep(state, N, beta, h, a, Tinf)
...
invTransformParams <- function(posterior)
{
    ...
    posterior$R0 = posterior$beta * posterior$Tinf
...
calclogl <- function(params) {
    ...
    logPriorP <- logPriorP + dexp(h0, 0.01, log=T)
    ...
    logPriorP <- logPriorP + dnorm(Tinf, mean=5, sd=5, log=T)
...
fit.paramnames <- c("beta", "logHR", "logHRDR", "HL", "DL",
                    "Tinf", "h0", "ht", "Tinc", "lockdownmort")
keyparamnames <- c("beta", "IFR", "Tinf", "h0", "ht", "Tinc", "R0", "Rt")
fitkeyparamnames <- c("beta", "logHR", "logHRDR", "Tinf", "Tinc", "h0", "ht")

init <- c(1, log(0.02), log(0.3), 10, 20, 5, 0, 0, 8, total_deaths_at_lockdown,
          rep(0.9, length(Es.time)))
scales <- c(1, 0.05, 0.1, 1, 1, 0.3, 0.01, 0.01, 0.3, total_deaths_at_lockdown / 20,
