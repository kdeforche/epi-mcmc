...
calculateModel <- function(params, period)
{
    ...
    if (length(lds) > 0) {
        ...
        parms <- c(N = N, a = a,
                   ...
                   ef2s = data_offset + lockdown_offset + lockdown_transition_period,
                   ef2e = data_offset + lockdown_offset + lockdown_transition_period + 10,
                   betat0 = betat0, Tinft0 = Tinft0,
                   betat1 = betat1, Tinft1 = Tinft1,
                   betat2 = betat2, Tinft2 = Tinft2,
                   betat3 = betat3, Tinft3 = Tinft3)
...
calclogp <- function(params) {
    ...
    logPriorP <- logPriorP + dnorm(Rt2 - Rt3, mean=0, sd=2, log=T)
    logPriorP <- logPriorP + dnorm(HLsd, mean=5, sd=1, log=T)
    logPriorP <- logPriorP + dnorm(DLsd, mean=5, sd=1, log=T)
    logPriorP <- logPriorP + dnorm(died_latency, mean=21, sd=3, log=T)
...
fit.paramnames <- c("Rt0", "Rt1", "Rt2", "Rt3", "HR", "HL", "DL", "phs_morts", "phs", "HLsd", "DLsd")
keyparamnames <- c("Rt0", "Rt1", "Rt2", "Rt3", "phs")
fitkeyparamnames <- keyparamnames
init <- c(2.9, 1.2, 0.9, 0.9, 0.02, 10, 27, total_deaths_at_lockdown, 0, 5, 5)

df_params <- data.frame(name = fit.paramnames,
                        min = c(0.1, 0.1, 0.1, 0.1, 0.001, 5, 10, 0, -30, 2, 2),
