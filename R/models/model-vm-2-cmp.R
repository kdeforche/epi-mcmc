...
calculateModel <- function(params, period)
{
    ...
    ldo <- lockdown_offset + lockdown_transition_start_var
    ldtp <- lockdown_transition_period - lockdown_transition_start_var + lockdown_transition_end_var
    
    for (i in (padding + 1):(padding + period)) {
        betaIn = calcpar(i - data_offset, betaIn0, betaInt, Es, ldo, ldtp)
        betaIs = calcpar(i - data_offset, betaIs0, betaIst, Es, ldo, ldtp)
        Tin = calcpar(i - data_offset, Tin0, Tint, Es, ldo, ldtp)
...
keyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "R0", "Rt", "G0", "Gt",
                   "Rin0", "Ris0")
fitkeyparamnames <- c("R0", "Rt", "G0", "Gt", "Tin0", "Tint", "ldtsv", "ldtev")

init <- c(2.9, 0.9, 5, 5, 2, 1, log(0.02), 10, 20, total_deaths_at_lockdown, 0, 0,
          rep(0.9, length(Es.time)))
scales <- c(1, 1, 1, 1, 1, 1, 0.05, 1, 1, total_deaths_at_lockdown / 20, 5, 5,
