...
calculateModel <- function(params, period)
{
    ...
    Tint <- params[5]
    hosp_rate <- params[6]
    hosp_latency <- params[7]
    died_latency <- params[8]
...
fit.paramnames <- c("R0", "Rt", "Tef0", "Tin0", "Tint",
                    "logHR", "HL", "DL", "lockdownmort")
keyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "R0", "Rt", "Tef0",
                   "Rin0", "Ris0")
fitkeyparamnames <- c("R0", "Rt", "Tef0", "Tin0", "Tint")

init <- c(2.9, 0.9, 3, 2, 1, log(0.02), 10, 20, total_deaths_at_lockdown,
          rep(0.9, length(Es.time)))
scales <- c(1, 1, 1, 1, 1, 0.05, 1, 1, total_deaths_at_lockdown / 20,
