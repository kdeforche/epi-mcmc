...
calculateModel <- function(params, period)
{
    ...
    for (i in (padding + 1):(padding + period)) {
        ...
        if (i - data_offset > lockdown_offset + lockdown_transition_period) {
            beta = max(0, beta + (i - data_offset - lockdown_offset - lockdown_transition_period) * betaf)
        }
...
fit.paramnames <- c("beta0", "betat", "logHR", "HL", "DL", "lockdownmort", "betaf")
keyparamnames <- c("beta0", "betat", "R0", "Rt", "betaf")
fitkeyparamnames <- c("beta0", "betat", "betaf")

init <- c(1.2, 0.3, log(0.02), 10, 20, total_deaths_at_lockdown, 0,
          rep(0.9, length(Es.time)))
scales <- c(1, 1, 0.05, 1, 1, total_deaths_at_lockdown / 20, 0.01,
