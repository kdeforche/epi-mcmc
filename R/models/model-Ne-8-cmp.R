...
calclogl <- function(params) {
    ...
    logPriorP <- logPriorP + dnorm(betaIs0, 0, 0.5, log=T)
    logPriorP <- logPriorP + dnorm(betaIst, 0, 0.5, log=T)
...
fit.paramnames <- c("logBetaIn0", "logBetaInt", "logBetaIs0", "logBetaIst", "logHR", "HL", "DL",
                    "lockdownmort", "kt", "logGamma.in0", "logGamma.int")
keyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "R0", "Rt", "kt")
fitkeyparamnames <- c("logBetaIn0", "logBetaInt", "logBetaIs0", "logBetaIst", "kt", "logGamma.in0", "logGamma.int")

init <- c(log(1), log(1), log(0.1), log(0.1), log(0.02), 10, 20, total_deaths_at_lockdown, 0.5, log(1/7), log(1/2),
          rep(0.9, length(Es.time)))
scales <- c(1, 1, 1, 1, 0.05, 1, 1, total_deaths_at_lockdown / 20, 0.3, 1, 1,
