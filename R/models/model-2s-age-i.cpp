...
#define t14        parms[66]      // step change point
#define betay14    parms[67]
#define betao14    parms[68]
#define betayo14   parms[69]
#define t15        parms[70]      // step change point
...
#define t18        parms[82]      // step change point
#define betay18    parms[83]
#define betao18    parms[84]
#define betayo18   parms[85]

const int wzc_vacc_start = 306; // 2021-01-22
const int wzc_vacc_end = 336;   // 2021-02-22
const double wzc_vacc_rate = 8387.1;

const int o_vacc_start = 386;
const int o_vacc_end = 417;
const double o_vacc_rate = 75048.39;

const int y_vacc_start = 421;
const int y_vacc_end = 498;
const double y_vacc_rate = 88831.17;
...
static double interpolate(double t,
			  ...
			  double vt16, double vt17, double vt18)
{
  if (t > t18)
    return vt18;
  else if (t > t17)
...
extern "C" {
	       ...
	       double *yout, int *ip)
  {
    ...
    double ygot_vaccinated = 0;

    if (*t >= t2 + y_vacc_start && *t < t2 + y_vacc_end)
      ygot_vaccinated = y_vacc_rate * 0.5 * 0.8; // 50% already immune
    ...
    yout[1] = 1/gamma * (wt_ygot_infected + wt_ogot_infected) / (wt_Iy + wt_Io);
    if (mt_Iy + mt_Io > 10)
      yout[2] = 1/gamma * (mt_ygot_infected + mt_ogot_infected) / (mt_Iy + mt_Io);
    else
      yout[2] = 0;
    yout[3] = yout[0] * (Ny + No) / (Sy + So);

    yout[4] = 1/gamma * ((betay * wt_Iy + mt_f * betay * mt_Iy) / Ny * Sy + (betayo * wt_Iy + mt_f * betayo * mt_Iy) / Ny * So) / (wt_Iy + mt_Iy);
    yout[5] = 1/gamma *                                                     ( betao * wt_Io + mt_f * betao * mt_Io ) / No * So  / (wt_Io + mt_Io);

    yout[6] = wt_ygot_infected;
    yout[7] = mt_ygot_infected;
    yout[8] = wt_ogot_infected;
    yout[9] = mt_ogot_infected;

    if (wt_Iy + mt_Iy > 0)
      yout[10] = (betay * wt_Iy + mt_f * betay * mt_Iy) / (wt_Iy + mt_Iy);
    else
      yout[10] = betay;

    if (wt_Io + mt_Io > 0)
      yout[11] = (betao * wt_Io + mt_f * betao * mt_Io) / (wt_Io + mt_Io);
    else
      yout[11] = betao;

    if (wt_Iy + mt_Iy > 0)
      yout[12] = (betayo * wt_Iy + mt_f * betayo * mt_Iy) / (wt_Iy + mt_Iy);
    else
      yout[12] = betayo;
  }
}
