...
simstep <- function(state, N, beta, a, gamma, k)
{
    i = state$i
    ...
    Ne = max(EIR, k * N)
    got_infected = beta * state$I[i] / N * (Ne - EIR)
...
fit.paramnames <- c("beta", "logHR", "logHRDR", "HL", "DL",
                    "Tinf", "Tinc", "lockdownmort", "kt")
keyparamnames <- c("beta", "IFR", "Tinf", "Tinc", "R0", "Rt", "kt")
fitkeyparamnames <- c("beta", "logHR", "logHRDR", "Tinf", "Tinc", "kt")

init <- c(1, log(0.02), log(0.3), 10, 20, 5, 8, total_deaths_at_lockdown, 0.1,
          rep(0.9, length(Es.time)))
scales <- c(1, 0.05, 0.1, 1, 1, 0.3, 0.3, total_deaths_at_lockdown / 20, 1,
