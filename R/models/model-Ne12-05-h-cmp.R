...
calclogl <- function(params) {
    ...
    ## Rond een willekeurige mu met zoveel variantie als nodig
    ## Maar welk getal dan? Getal dicht bij 2.8 ? truncated normal
    ## distribution
    logPriorP <- logPriorP + dnorm(Teft, mean=Teftmu, sd=0.2, log=T)
    logPriorP <- logPriorP + dunif(Teftmu, 0, 2.8, log=T)
...
fit.paramnames <- c("RIn0", "RInt", "R0", "Rt", "logHR", "HL", "DL",
                    "lockdownmort", "gamma.in0", "gamma.int", "Teftmu")
keyparamnames <- c("betaIn0", "betaInt", "betaIs0", "betaIst", "R0", "Rt", "Tef0", "Teft",
                   "RIn0", "Teftmu")
fitkeyparamnames <- c("R0", "Rt", "RIn0", "RInt", "gamma.in0", "gamma.int", "Teftmu")

init <- c(2.9, 0.9, 3, 1, log(0.02), 10, 20, total_deaths_at_lockdown, 1/7, 1/2, 2.8,
          rep(0.9, length(Es.time)))
scales <- c(1, 1, 1, 1, 0.05, 1, 1, total_deaths_at_lockdown / 20, 2, 0.3, 1,
