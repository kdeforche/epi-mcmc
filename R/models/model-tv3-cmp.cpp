...
#define ef2s parms[5]
#define ef2e parms[6]
#define betat0 parms[7]
#define Tinft0 parms[8]
#define betat1 parms[9]
#define Tinft1 parms[10]
#define betat2 parms[11]
#define Tinft2 parms[12]
#define betat3 parms[13]
#define Tinft3 parms[14]
...
static double interpolate(double t, double vt0, double vt1, double vt2, double vt3)
{
  if (t < phts)
    return vt0;
  else {
	...
	if (t < ef2s) {
	  ...
	  if (t < ef2e) {
	    return vt2 + (t - ef2s) / (ef2e - ef2s) * (vt3 - vt2);
	  } else {
	    return vt3;
	  }
	}
...
extern "C" {
	       ...
	       double *yout, int *ip)
  {
    ...
    const double beta = interpolate(*t, betat0, betat1, betat2, betat3);
    const double Tinf = interpolate(*t, Tinft0, Tinft1, Tinft2, Tinft3);
