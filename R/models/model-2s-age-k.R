...
calculateModel <- function(params, period)
{
    ...
    state$wt.Re <- rep(0, padding + period)
    state$mt.Re <- rep(0, padding + period)
    ...
    outnames <- c("Re", "wt.Re", "mt.Re", "Rt", "y.Re", "o.Re", "wt.y.i", "mt.y.i", "wt.o.i", "mt.o.i",
    ...
    state$y.i[(padding + 1):(padding + period1)] = out[,24] + out[,25]
    state$o.i[(padding + 1):(padding + period1)] = out[,26] + out[,27]
    ...
    state$wt.Re[(padding + 1):(padding + l.out)] = out[,19]
    state$mt.Re[(padding + 1):(padding + l.out)] = out[,20]
    state$Rt[(padding + 1):(padding + l.out)] = out[,21]
    state$y.Re[(padding + 1):(padding + l.out)] = out[,22]
    state$o.Re[(padding + 1):(padding + l.out)] = out[,23]
    state$wt.y.i[(padding + 1):(padding + l.out)] = out[,24]
    state$mt.y.i[(padding + 1):(padding + l.out)] = out[,25]
    state$wt.o.i[(padding + 1):(padding + l.out)] = out[,26]
    state$mt.o.i[(padding + 1):(padding + l.out)] = out[,27]
    state$y.i[(padding + 1):(padding + l.out)] = out[,24] + out[,25]
    state$o.i[(padding + 1):(padding + l.out)] = out[,26] + out[,27]
    state$y.beta[(padding + 1):(padding + l.out)] = out[,28]
    state$o.beta[(padding + 1):(padding + l.out)] = out[,29]
    state$yo.beta[(padding + 1):(padding + l.out)] = out[,30]

    state$mt.E[(padding + 1):(padding + l.out)] = out[,6] + out[,7] + out[,14] + out[,15]
    state$mt.I[(padding + 1):(padding + l.out)] = out[,8] + out[,16]

    gycr <- pmin(1, ycase_rate * y.hr * (1 + (fyifr - 1) * g))     ## fyifr: age reduction
    gyifr <- y.ifr * (1 + (fyifr - 1) * g) * (1 - ifrred * gtrimp)  ## ifrred: treatment improvement -> on lowers IFR 
    gyhr <- yhosp_rate * y.hr * (1 + (fyhr - 1) * g)

    gocr <- pmin(1, ocase_rate * o.hr)
...
{
    posterior$easter = posterior$betay15 / posterior$betay14
    posterior$exit.1 = posterior$betay16 / posterior$betay15
    posterior$exit.2 = posterior$betay17 / posterior$betay16
...
calclogp <- function(params, misc) {
    ...
    logPriorP <- logPriorP + dnorm(t10o, mean=d10, sd=21, log=T)
    logPriorP <- logPriorP + dnorm(t11o, mean=d11, sd=21, log=T)
    ...
    logPriorP <- logPriorP + dnorm(t16o, mean=duncertain, sd=14, log=T)
    ...
    logPriorP <- logPriorP + dlnorm(betay0/betay1, 0, lSD, log=T)
    logPriorP <- logPriorP + dlnorm(betao0/betao1, 0, lSD, log=T)
    logPriorP <- logPriorP + dlnorm(betayo0/betayo1, 0, lSD, log=T)
    ...
    logPriorP <- logPriorP + dlnorm(betay17/betay16, log(1.15), log(1.075), log=T)
    logPriorP <- logPriorP + dlnorm(betao17/betao16, log(1.15), log(1.075), log=T)
    logPriorP <- logPriorP + dlnorm(betayo17/betayo16, log(1.15), log(1.075), log=T)
